...
export class Image {
  ...
  static make_image(
    ...
    let result_components: number | null = null;
    ...
    for (let target_y = 0; target_y < scaled_width; target_y += 1) {
      for (let target_x = 0; target_x < scaled_height; target_x += 1) {
        ...
        result_colors[target_offset] = Math.floor(
          clamped(colors[source_offset]) * 255,
        );
        result_colors[target_offset + 1] = Math.floor(
          clamped(colors[source_offset + 1]) * 255,
        );
        result_colors[target_offset + 2] = Math.floor(
          clamped(colors[source_offset + 2]) * 255,
        );
